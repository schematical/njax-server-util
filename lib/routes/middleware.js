...
module.exports = function (app) {
	...
	function middleware(req, res, next) {
		...
		res.render = function (tpl, data) {
			...
			for (var i in app.njax.env_config.tpl_dirs) {
				var tpl_path = path.join(app.njax.env_config.tpl_dirs[i], tpl + '.hjs');
				if (fs.existsSync(tpl_path)) {
					//compile bootstrap
					tpl = path.join(app.njax.env_config.tpl_dirs[i], tpl);
		...
		res._bootstrap = {};
		res._bootstrap.cookie = _.clone(app.njax.env_config.cookie);
		...
		res.bootstrap(
				'req',
				{
					host: req.host,
					path: req.path,
					domain: app.njax.env_config.domain
				}
		);
		//First determine calltype
		req.njax.call_type = is_api_call(req) ? 'api' : 'www';
		var host = req.get('x-forwarded-host') || ((req.hostname || req.host) + ':' + app.njax.env_config.port);


		var www_url = app.njax.env_config.domain;
		if (!app.njax.env_config.hide_port) {
			www_url += ':' + app.njax.env_config.port;
		}

		//TODO Check env_config instead of auto populating


		res.bootstrap('core_api_url', app.njax.env_config.core.api.protocol + '://' + app.njax.env_config.core.api.host);
		res.bootstrap('core_www_url', app.njax.env_config.core.api.protocol + '://' + app.njax.env_config.core.host);
		res.bootstrap('api_url', app.njax.env_config.api_url);
		res.bootstrap('www_url', app.njax.env_config.www_url);
		res.bootstrap('client_id', app.njax.env_config.client_id);
	...
	app.use(function (req, res, next) {
		...
		if (!req.session || !req.session._cache) {
			return next();
		}
