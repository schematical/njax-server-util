...
var mongo_session = require('./modules/mongo_session');
var env_config = {
    aws:{
        accessKeyId:'xx',
        secretAccessKey:'xxx',
...
module.exports = njax = njax_util = {
        app:function(njax_config, options){
           ...
           /* app.locals.partials = {
			...
			//app.use(mongo_session(app, { secret: env_config.cookie.secret, cookie:  app.njax.env_config.cookie}))
        ...
        setup_partials:function(partial){
			...
			if(env_config.create_missing_tpls){
				var file_loc = path.join(env_config.tpl_dirs[0], partial + '.hjs');
				console.log("Writing:", file_loc);
				var file_dir = path.dirname(file_loc);
				if(!fs.exists(file_dir)){
					mkdirp.sync(file_dir);
				}
				return fs.writeFileSync(file_loc, partial);
