...
module.exports = function(app){
	//app.njax.mongo_session = function() {
		...
		var connection = app.mongoose.createConnection(
			app.njax.config.session_mongo || app.njax.config.mongo
		);
		var fields = {
		...
		/*
		 secret: app.njax.config.cookie.secret,
		 name:'njax.mongo.session',
		 cookie: {
		 expires:false,
		 domain:app.njax.config.cookie.domain,
		 },
		 */
		...
		return function (req, res, next) {
			...
			var cookie_name =app.njax.config.name || _MongoSession.default_cookie_name;
			//Grab cookie
			...
			return async.series([
					...
					function (cb) {
						if (mongo_doc) {
							...
							mongo_data =  new _MongoSession.Session(raw_data);
							return cb();
						}

						mongo_data = new _MongoSession.Session({});
						...
						//console.log("Creating New Cookies - Path:", req.path);
						return cb();
					},
					//Setup the basics
					function(cb){
						...
						onHeaders(res, function setHeaders() {
							...
							if (!cookie || cookie !=  mongo_doc.key) {
								...
								res.cookie(cookie_name, mongo_doc.key, app.njax.config.cookie);
							}
