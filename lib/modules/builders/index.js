...
module.exports = function(app){
	...
	var buildMaster = {
		...
		prepairModel: function (model) {
			...
			model.displayName = model.displayName || buildMaster._camelcaseToSpaces(model.capitalName);
			if (model.parent) {
			...
			while(!has_no_parent && danger < 15){
				...
				if(_focus_model.parent){
					has_no_parent = false;
					_focus_model = buildMaster.models[_focus_model.fields[_focus_model.parent].ref];
				}else{
					has_no_parent = true;
				}
			...
			for(var i in lineage){
				...
				if(lineage[i].name != model.name) {
					uri_prefix += '/:' + lineage[i].name;
				}
			}
			model.full_url_prefix = uri_prefix;
