...
module.exports = function (app) {
	var RouteBuilder = {
		build: function (buildMaster) {
			...
			app.njax.routes.model = app.njax.routes.model = {} || {};
			for (var i in buildMaster.models) {
				var model = buildMaster.models[i];
				app.njax.routes.model[model.name] = RouteBuilder.buildModel(model);
			}
		},
		buildModel: function (_model) {
			...
			route.initPartials = function () {
				//app.locals.partials['_' + _model.name + '_edit_form'] = 'model/' + _model.name + '/_edit_form';
				//app.locals.partials['_' + _model.name + '_list_single'] = 'model/' + _model.name + '/_list_single';
			}
...
//console.log("Creating Route:", uri + '/:' + _model.name.toLowerCase());
				app.all(
					uri + '/:' + _model.name.toLowerCase(),
					route.render_detail_full
				);
			...
			route.populate = function (req, res, next, id) {
				//console.log("ModelName:", _model.name, "ID:" , id);
					...
					if (_model.parent) {
						if (_model.parent == 'owner') {
							...
							if (_model.fields[_model.parent].bootstrap_populate) {
								var field_name = _model.fields[_model.parent].bootstrap_populate;

								query[_model.parent] = req[field_name]._id;
							}
						}
					}

					return app.model[_model.capitalName].findOne(query, function (err, collection) {
						if (err) {
							return next(err);
						}

						if (collection) {
							res.bootstrap(_model.name, collection);
						}
						return next();
					});
				} else {
			...
			route.render_remove = function (req, res, next) {
				return res.render('index', res.locals[_model.name]);
				//return res.render('model/' + _model.name + '/detail', res.locals[_model.name]);
			};
			route.render_list = function (req, res, next) {
				return res.render('index', res.locals[_model.name + 's']);
				//return res.render('model/' + _model.name + '/list', res.locals[_model.name + 's']);
			...
			route.render_detail = function (req, res, next) {
				...
				return res.render('index', req[_model.name].toObject());
				//return res.render('model/' + _model.name + '/detail', req[_model.name].toObject());
			};
			...
			route.render_edit = function (req, res, next) {
				async.series([
					...
					function (cb) {
						...
						return res.render('model/' + _model.name + '/edit');
					}
				]);
			},
			...
			//Here we go
			/*
			 Extend the app in NJax
			 */
			var route_extend = path.join(app.njax.env_config.njax_dir, 'lib', 'routes', 'model', _model.name);
			if(fs.existsSync(route_extend + '.js')){
				require(route_extend)(app).extend(route, _model);
			}

			/*
			 Extend the root app
			 */
			var route_extend = path.join(app.njax.env_config.app_dir, 'lib', 'routes', 'model', _model.name);
			if(fs.existsSync(route_extend + '.js')){
				require(route_extend)(app).extend(route, _model);
			}
