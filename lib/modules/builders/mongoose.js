	...
	var MongooseBuilder = {
		...
		prepairModel: function (model) {
			for (var key in model.fields) {
				model.fields[key] = MongooseBuilder.prepairModelField(key, model.fields[key], model)
			}
		},
		prepairModelField: function (key, fieldData, model) {
			var Schema = app.mongoose.Schema;
			if(!fieldData){
				console.error(model.name, key, fieldData);
			...
			switch (fieldData.type.toLowerCase()) {
				...
				case 'ref':
					...
					fieldData.mongo_type = {type: Schema.Types.ObjectId, ref: this.capitalizeFirstLetter(fieldData.ref)}
					break
				case 'core-ref':
				case 'core_ref':
		...
		prepairModelFields: function (model) {
			for (var key in model.fields) {
				MongooseBuilder.prepairModelFields(key, model.fields[key], model);
			}
		},
		build: function (buildMaster) {
			...
			for (var i in buildMaster.models) {
				...
				var upperName = MongooseBuilder.capitalizeFirstLetter(model.name);
		...
		buildModel: function (model) {
			...
			var fields = {};
			for (var key in model.fields) {
				fields[key] = model.fields[key].mongo_type;
			}
			console.log(fields);
			...
			newSchema.virtual('uri').get(function () {
				...
				if (!this.target) {
					...
					var parent_uri = (this.target.uri || (this._parent_uri));
				}
				return parent_uri + '/' + model.uri_prefix + '/' + this._id;
				...
				if (_model.fields[name].type == 's3-asset') {
					newSchema.virtual(field.name + '_s3').get(function () {
						...
						if (!app.njax.config.local_file_cache) {
							var url = '//s3.amazonaws.com/' + app.njax.config.aws.bucket_name + '/' + this[field.name];
						} else {
							var url = app.njax.config.www_url + '/cache/' + this[field.name];
						}
