	...
	var MongooseBuilder = {
		...
		build: function (buildMaster) {
			...
			for (var i in buildMaster.models) {
				...
				var newSchema = MongooseBuilder.buildModel(model, buildMaster);
		...
		buildModel: function (_model, buildMaster) {
			...
			newSchema.pre('save', function (next) {
				...
				if (_model.parent) {
					if (!this._parent_uri) {
						...
						var _this = this;
						var parent_ref = _model.fields[_model.parent].ref;
						var parent = buildMaster.models[parent_ref];
						console.log("parent:", parent);
						return app.model[parent.capitalName].findOne({_id: this[_model.parent]}).exec(function (err, instance) {
							...
							_this._parent_uri = instance.uri;
							return next();
						});
			 ...
			 */
			var model_extend = path.join(app.njax.env_config.njax_dir, 'lib', 'model', _model.name);
			if(fs.existsSync(model_extend + '.js')){
				require(model_extend)(app).extend(newSchema);
			}

			/*
			Extend the root app
			 */
			var model_extend = path.join(app.njax.env_config.app_dir, 'lib', 'model', _model.name);
			if(fs.existsSync(model_extend + '.js')){
				require(model_extend)(app).extend(newSchema);
			}
